...
@endverbatim
  ...
  *current* system time, but we really want the MidiTime(x) for some
  ...
  \par MIDI while scrubbing / play at speed
...
enum {
   ...
   // When scrubbing or using play at speed, we want to emit
   // events immediately instead, so pass this to Pm_OpenOutput
   MIDI_IMMEDIATE_EVENTS = 0
};
...
bool AudioIO::StartPortMidiStream()
{
   ...
   int latency = MIDI_MINIMAL_LATENCY_MS;
#ifdef EXPERIMENTAL_SCRUBBING_SUPPORT
   if (mPlaybackSchedule.mPlayMode == PlaybackSchedule::PLAY_AT_SPEED) {
      latency = MIDI_IMMEDIATE_EVENTS;
   }
#endif
...
#ifdef EXPERIMENTAL_SCRUBBING_SUPPORT
   if (mPlaybackSchedule.mPlayMode == PlaybackSchedule::PLAY_AT_SPEED) {
      return mPlaybackSchedule.mTime;
   }
#endif
...
PmTimestamp AudioIO::MidiTime()
{
   ...
   // means than specifying it to Pm_OpenOutput.  The use of the accumulated
